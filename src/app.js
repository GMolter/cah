// /src/app.js
export const APP_BUILD = "APP_JS_BUILD 2025-09-01T06:20Z";
document.title = "CA(D)H";
...
const must = (s)=>{
  const el = $(s);
  if(!el){ console.error(`[DOM] Missing element ${s}`); }
  return el;
};
...
function showModal(show){
  const joinModal = must("#join-modal");
  if(joinModal){ joinModal.style.display = show ? "flex" : "none"; }
...
async function joinRoom(code, desiredName){
  ...
  // 3) Attempt to host (will fail if already set by someone else)
  ...
  // 4) createdAt (set-once, safe to ignore failures if it exists)
...
document.addEventListener("DOMContentLoaded", async ()=>{
  ...
  // DOM hooks (guarded)
  const joinModal = must("#join-modal");
  const joinBtn   = must("#join-btn");
  const roomInput = must("#room-input");
  const nameInput = must("#name-input");
  const chatInput = must("#chat-input");
  const chatSend  = must("#chat-send");

  // Prefill
  const params = new URLSearchParams(location.search);
  const qsRoom = (params.get("room") || "").toUpperCase();

  if(nameInput) nameInput.value = localStorage.getItem("cadh-name") || defaultName();
  ...
  ui = new UI(store, {
    ...
    onPlayCard: playCard,
    onJudgePick: (submissionId)=> { if (canIHost()) hostStartRound(); /* or call host pick flow */ },
    onStartRound: hostStartRound
  });

  // Reflect room code in header when it changes
  store.subscribe(()=> ui.setRoom(currentRoom));
